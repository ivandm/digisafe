...
async function load_markers( url, favorite=false ) {
    //console.log(url);
    const qs = {{jobs|safe}};
    var arrStr = "search="+qs.join("&search=")
    const d_in = "{{sessionbook.start_date|date:'Y-m-d'}}";
    const d_out = "{{sessionbook.end_date|date:'Y-m-d'}}";
    const days = $("input#days_input").val();
    const busy = $("input#busy_input").is(":checked");
    const query_str = `in_bbox=${map.getBounds().toBBoxString()}&${arrStr}&date_in=${d_in}&date_out=${d_out}&favorite=${favorite}&days=${days}&busy=${busy}`;
...
async function render_markers() {
    ...
    var url = "{{api_markers_defaultposition_url2}}";
    ...
    var url = "{{api_markers_agenda_url2}}";
    var markers = await load_markers( url);
    ...
    var url = "{{api_markers_agendabusy_url2}}";
    var markers = await load_markers( url);
...
/* Manage search parameters. Extra days etc. */
/* EVENTS Extra Days */
/* Days input event */
...
function drawChart() {
    ...
    var Rows = [
        ["Range","Search date range", "ligthblue", "", new Date("{{sessionbook.start_date|date:'Y-m-d'}}"+" 00:01"),new Date("{{sessionbook.end_date|date:'Y-m-d'}}"+" 23:59")]
      ];
...
function addRowsFromDefaultPosition(markers){
    ...
    markers.features.forEach(function(item, index){
        ...
        var d_start = new Date( "{{sessionbook.start_date|date:'Y-m-d'}}"  + " 00:01");
        var d_end   = new Date( "{{sessionbook.end_date|date:'Y-m-d'}}" + " 23:59");
        var d_start_display = `${d_start.getDate()}-${d_start.getMonth()+1}-${d_start.getFullYear()}`;
        var d_end_display = `${d_end.getDate()}-${d_end.getMonth()+1}-${d_end.getFullYear()}`;
